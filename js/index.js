...
function confirmTrip(e) {
    ...
    $('#page-create #frm-confirm #budget').text($('#page-create #frm-register #budget').val());
    $('#page-create #frm-confirm #tripType').text($('#page-create #frm-register #tripType').val());
...
function registerTrip(e) {
    ...
    let budget = $('#page-create #frm-register #budget').val();
    let tripType = $('#page-create #frm-register #tripType').val();

    db.transaction(function (tx) {
        let query = `INSERT INTO Trip (Name, Destination, Description, Risk, Date, Budget, TripType) VALUES (?, ?, ?, ?, julianday('${date}'), ?, ?)`;
        tx.executeSql(query, [name, destination, description, risk, budget, tripType], transactionSuccess, transactionError);
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            if (result.rows[0] != null) {
                ...
                $('#page-detail #detail #budget').text(result.rows[0].Budget);
                $('#page-detail #detail #tripType').text(result.rows[0].TripType);
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            if (result.rows[0] != null) {
                ...
                $(`#page-detail #frm-update #budget`).val(result.rows[0].Budget);
                $(`#page-detail #frm-update #tripType`).val(result.rows[0].TripType);
...
function updateTrip(e) {
    ...
    let budget = $('#page-detail #frm-update #budget').val();
    let tripType = $('#page-detail #frm-update #tripType').val();

    db.transaction(function (tx) {
        let query = `UPDATE Trip SET Name = ?, Destination = ?, Description = ?, Risk = ?, Date = julianday('${date}'), Budget = ?, TripType = ? WHERE Id = ?`;

        tx.executeSql(query, [name, destination, description, risk, budget, tripType, id], transactionSuccess, transactionError);
